...
本机 Web：打开一部带软字幕的番，AI 看着同一帧听你吐槽。本集剧透不卡，上下文必须跟上播放进度。

> **V0 已出炉。** 现状与拍板以 `docs/交接文档.md` 为准。
...
| 什么 | 哪条事件 |
|------|----------|
| 人设 + 本集完整剧情 | 开场一次 `session.update.instructions`（可含整集 JSON） |
...
2. plot 抽轨 + 解析 → 对白 JSON（`POST /api/plot/subtitles`；没有软字幕轨就报错停）  
3. plot 富化 + 缓存 → 剧情 JSON（`POST /api/plot/enrich`；官方 json_object，不设 max_tokens）  
4. realtime 中继 → 会话握手（`GET /api/realtime`；allowlist + 逐事件日志）  
5. 前端麦 / 抓帧 / 段切注入（随第 4 步一起落地，看控制台 `[withyou]` 与 Go 日志）  
