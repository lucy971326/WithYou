...
**一句话**：做一个"AI 陪伴你看番/看剧"的本机 Web 应用 —— 打开带软字幕轨的视频，Realtime 模型结合画面与剧情，在你吐槽时低延迟秒回。本集剧透不卡；要卡住的是「现在播到哪」。
...
2. **运行时注入**（分级、省 token；本集剧透允许）：
   - 开始播放 → 注入**本集完整剧情 JSON**（模型先有全图，才跟得上）
   - 每到一个剧情段 → 再注入**当前 sub_segment**（注意力拉到「此刻」）
...
| 潜在问题 | 现象/风险 | 当前解方 |
|---------|----------|---------|
| **剧情幻觉** | 裸模型自信编造集数/剧情/角色互动 | 离线富化剧情 JSON 作为事实源；开场灌整集，段切再灌当前节。不卡本集剧透 |
